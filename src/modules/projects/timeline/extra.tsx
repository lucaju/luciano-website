...
export const Item = ({
	...
	return (
		<div className={cn('text-xs', pending && 'text-muted-foreground')}>
			...
			{type === 'Book Chapter' && (
				<BookOpenText className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Thesis' && (
				<GraduationCapIcon className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Journal Article' && (
				<FileText className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Proceedings' && (
				<NotebookText className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Conference Panel' && (
				<Component className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Conference Paper' && (
				<Lectern className='inline-block mr-1 h-3 w-3 align-middle mb-0.75' />
			)}
			{type === 'Exhibition' && (
				<Presentation className='inline-block mr-1 h-3 w-3 align-middle mb-0.25' />
			)}
			{type === 'Award' && <Trophy className='inline-block mr-1 h-3 w-3 align-middle mb-0.5' />}
			{type === 'Guest Speaker' && (
				<Podcast className='inline-block mr-1 h-3 w-3 align-middle mb-0.5' />
			)}
